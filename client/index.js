(function() {
    ...
    class Joystick {
        constructor(centerX, centerY, innerRadius, outerRadius) {
            console.assert(innerRadius < outerRadius, "Inner radius must be less than outer radius");
        ...
        draw() {
            ...
            ctx.arc(this.centerX + distance * Math.cos(angle), this.centerY + distance * Math.sin(angle), this.innerRadius, 0, 2 * Math.PI);
            ctx.fill();
            ctx.stroke();
            ...
            ctx.fillText(`(${x.toFixed(2)}, ${y.toFixed(2)})`, this.centerX - measure.width / 2, this.centerY - this.outerRadius - 10);
        }

        cartesianValue() {
            const { angle, distance } = this.polarValue();
            return { x: distance * Math.cos(angle) / (this.outerRadius - this.innerRadius), y: distance * Math.sin(angle) / (this.outerRadius - this.innerRadius) };
        }
        ...
        ontouchstart(event) {
            ...
            const ourTouch = Array.from(event.changedTouches)
                .find(touch => Math.hypot(touch.pageX - this.centerX, touch.pageY - this.centerY) < this.outerRadius);
        ...
        ontouchmove(event) {
            if (this.currentTouch === null) return;
            const ourTouch = Array.from(event.changedTouches)
                .find(touch => touch.identifier === this.currentTouch.identifier);
            if (ourTouch === undefined) return;
            this.currentTouch = copyTouch(ourTouch);
        }
    ...
    class Slider {
        ...
        ontouchstart(event) {
            if (this.currentTouch !== null) return;
            const ourTouch = Array.from(event.changedTouches)
                .find(touch => Math.abs(touch.pageX - this.centerX) < 20);
            if (ourTouch === undefined) return;
            this.currentTouch = copyTouch(ourTouch);
    ...
    const joystick = new Joystick(canvas.width / 2 - outerRadius, canvas.height / 2, innerRadius, outerRadius);
